...
shm_kind shm_mk_event_kind(const char *name, size_t event_size,
    ...
    for (size_t i = 0; i < ev_kinds_num; ++i) {
        if (strcmp(name, events_info[i].name) == 0) {
            assert(events_info[i].ev_size == event_size &&
                   "Different size for a known event");
    ...
    if (ev_kinds_num_allocated < ev_kinds_num) {
        ...
        events_info = realloc(events_info, sizeof(struct _ev_kind_rec) *
                                               ev_kinds_num_allocated);
    ...
    memcpy((char *)events_info[idx].signature, signature, strlen(signature));
...
void initialize_events() {
    ...
    dropped_kind = shm_mk_event_kind("dropped", sizeof(shm_event_dropped), "l");
    /* FIXME */
