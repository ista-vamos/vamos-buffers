...
struct monitor_buffer_node *attach_to_buffer(pid_t pid, pid_t tid,
                                             size_t size_in_pages) {
    char name[SHM_NAME_MAXLEN];
    if (shm_mapname_thread_pid_tid(name, pid, tid) == 0) {
        abort();
    }
    struct monitor_buffer_node *buffer_node =
        (struct monitor_buffer_node *)malloc(
            sizeof(struct monitor_buffer_node));
    ...
    void *mem =
        mmap(0, alloc_size, PROT_READ | PROT_WRITE, MAP_SHARED, buffer->fd, 0);
...
databuffer *attach_to_databuffer(pid_t pid, pid_t tid, size_t size_in_pages,
                                 uint64_t bufid) {
    char name[SHM_NAME_MAXLEN];
    ...
    void *mem = mmap(0, alloc_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    dbuf_node->fd = fd;
...
void insert_thread_buffer(monitored_process proc,
                          struct monitor_buffer_node *node) {
    ...
    while (proc->status == 0) {
        if (copy_events_wait(&proc->buffer, &entry, 1) == 1) {
            switch (entry.kind) {
                ...
                case ABMGMT_NEW: {
                    struct monitor_buffer_node *buffer_node = attach_to_buffer(
                        proc->pid, entry.payload32_1, entry.payload64_1);
                    insert_thread_buffer(proc, buffer_node);
                    proc->register_thread_buffer(&buffer_node->buffer);
                } break;
                case ABMGMT_NEWDATA: {
                    databuffer *dbuf = attach_to_databuffer(
                        proc->pid, entry.payload32_1, entry.payload64_1,
                        entry.payload64_2);
...
monitored_process only_attach_to_process(
    pid_t pid, int (*register_thread_buffer)(monitor_buffer buffer)) {
    char name[SHM_NAME_MAXLEN];
    ...
    buffer->buffer.fd =
        open(name, O_RDWR | O_NOFOLLOW | O_CLOEXEC | O_NONBLOCK, 0644);
    ...
    void *mem = mmap(0, alloc_size, PROT_READ | PROT_WRITE, MAP_SHARED,
                     buffer->buffer.fd, 0);
...
monitored_process attach_to_process(
    pid_t pid, int (*register_thread_buffer)(monitor_buffer buffer)) {
    monitored_process buffer =
        only_attach_to_process(pid, register_thread_buffer);
    thrd_create(&buffer->main_event_loop_thrd, &main_process_event_loop,
                (void *)buffer);

    return buffer;
}

size_t copy_events_wait(monitor_buffer buffer, buffer_entry *targetbuffers,
                        size_t count) {
    size_t actual = 0;
    ...
    while (actual < count && !buffer->stopped) {
        entry_id =
            atomic_load_explicit(&current_entry->id, memory_order_acquire);
        if ((entry_id & (buffer_entry_id)1) == 0) {
            if (entry_id == BUF_ENTRY_MAGIC_ID) {
                ...
                spin_wait();
                entry_id = atomic_load_explicit(&current_entry->id,
        ...
        *buffer_buffer = *current_entry;
        ...
        atomic_store_explicit(&current_entry->id, BUF_ENTRY_FREE_ID,
                              memory_order_release);
        if (buffer_buffer->flags == TBMGMT_DATA) {
            databuffer *dbuf =
                progress_data_buffer(buffer, buffer_buffer->payload64_1);
            size_t dsize = buffer_buffer->payload64_2;
            void *newmem = malloc(dsize + 1);
            memcpy(
                newmem,
                (void *)(((intptr_t)dbuf->start) + buffer_buffer->payload32_1),
                dsize);
            ((char *)newmem)[dsize] = 0;
...
// Writes at least one, but up to count entries from the event buffer into
// buffer_buffer, except if the monitoring buffer has been stopped. With very
// high probability, all these events are in sequence (the event monitored
// application would have to go through all possible 2^63 event IDs at least
// once, possibly multiple times to align within the event buffer, in order for
// the events at the current read location to have the same IDs as in an earlier
// run again while this function is collecting events).
size_t copy_events_nowait(monitor_buffer buffer, buffer_entry *buffer_buffer,
    ...
    buffer_entry_id entry_id;
    buffer_entry_id expected_entry_id =
        atomic_load_explicit(&prev_entry->id, memory_order_acquire) + 2;
    ...
    while (actual < count && !buffer->stopped) {
        ...
        if (entry_id != expected_entry_id) {
            if (entry_id == BUF_ENTRY_MAGIC_ID) {
                ...
                expected_entry_id_2 =
                    atomic_load_explicit(&prev_entry->id,
                                         memory_order_acquire) +
                    2;
