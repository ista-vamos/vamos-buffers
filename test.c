...
// globals code
STREAM_NumberPairs_out * arbiter_outevent;#include "./compiler/cfiles/intmap.h"
    ...
    if (are_events_in_head(BUFFER_P1, sizeof(STREAM_Primes_out), TEMPARR0, 1)) {
      ...
      if (true) {
        ...
        if (ln == rn) {
          ...
          intmap_clear( & buf);
          stream_args_P_0.pos = ln + 1;
          stream_args_P_1.pos = rn + 1;
        } else if (stream_args_P_0.pos <
          stream_args_P_1.pos
        ) {
          stream_args_P_0.pos = process(ln, lp, stream_args_P_0.pos, stream_args_P_1.pos);
          shm_arbiter_buffer_drop(BUFFER_P0, 1);

        } else {
          stream_args_P_1.pos = process(rn, rp, stream_args_P_1.pos, stream_args_P_0.pos);
          shm_arbiter_buffer_drop(BUFFER_P1, 1);
  ...
  if (chosen_streams != NULL) {
    ...
    if (are_events_in_head(BUFFER_F, sizeof(STREAM_Primes_out), TEMPARR3, 1)) {
      ...
      if (true) {
        ...
        stream_args_F.pos = process(n, p, stream_args_F.pos, stream_args_S.pos);
